...
> 测试基类 `AbstractIntegrationTest` 显式钉住用例依赖的规则参数（容量 6 / 槽长 30 分钟 /
...
- **容量**：按上海墙钟切 30 分钟槽（`app.slot.length-minutes` 可配），每槽全园区 6 个名额
  （`app.slot.capacity` 可配）；并发下单由事务级咨询锁按槽串行化，容量不超卖；
